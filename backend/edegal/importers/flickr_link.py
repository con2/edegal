...
def remove_known_suffixes(title: str):
    """
    >>> remove_known_suffixes("Korpkvädet (LARP)")
    'Korpkvädet'
    """
    for regex in KNOWN_SUFFIXES:
        if match := regex.search(title):
...
def import_flickr_link(
    path,
    flickr_url,
    override_title=None,
    override_slug=None,
    leaf_album_title=None,
    ...
    if import_item is not None:
        import_item.album = album
        import_item.status = "done"
        import_item.save()
