...
### As a visitor I want to download a single photo or an album of photos

When downloading photos, the visitor should be shown the conditions of use for those photos and asked to accept them before downloading.

Direct download of the preview photo should be prevented (current implementation uses `pointer-events: none` CSS).

Album download is zipped, current implementation stores them indefinitely.
...
There may be hundreds of photos uploaded in one session. The frontend should support this and

As photos are uploaded, previews and thumbnails for them are generated in the background.

If the album does not yet have a thumbnail, a photo from the first batch of photos uploaded should be selected as the thumbnail. The first landscape orientation photo is preferred as thumbnail, but if an album contains only portrait orientation photos, one of those must suffice. The owner of the album should be able to change the thumbnail of the album.
...
Larpit.fi ([source in GitHub](https://github.com/con2/larpit-fi)) represents my current state of the art in frontend development. I am quite happy with the tooling and DX in there, and it may be consulted for reference; though it may be further improved on.

## Hosting

The application will be self-hosted with Kubernetes.

Kubernetes deployment should let go of Skaffold and Emrichen and instead use Helm. Write an opinionated chart for our purposes with minimal templating. Infokala-tracon has [one such example](https://github.com/con2/infokala-tracon/tree/master/chart). Namespaces, see below.

No staging environment. Deploy straight into production.

Use Gateway instead of Ingress from the get-go.

## Technical notes

### Coexist with current version

The current version will continue serving alongside the new one until we are ready to commit to the switch.

We'll use the hostnames `uusi.conikuvat.fi` (Kubernetes namespace `conikuvat-v4`) and `uusi.larppikuvat.fi` (`larppikuvat-v4`) during the transitionary period.
...
Legacy content URLs under the new site should stay the same as old (save for the hostname during transitionary period).
...
This may not be a feasible approach for Next.js, but as a general guideline, browsing an album in order should not make a query (let alone several queries) for each picture accessed if reasonably possible.
...
Table and field names should be `lower_snake_case` (`@map` from `lowerCamelCase` for field names and `@@map` from `UpperCamelCase` for model names in Prisma schema).
...
### Use `@con2/components` for Markdown rendering and other shared components

The library lives in [GitHub](https://github.com/con2/components).

## Out of scope for the MVP

We'll design these separately later:
