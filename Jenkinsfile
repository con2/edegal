...
node {
  ...
  stage("Build frontend") {
    sh "cd frontend && rm -rf build && mkdir build && docker build --tag ${frontendImage} . && docker run --rm --volume ./build:/usr/src/app/build --env NODE_ENV=production ${frontendImage} yarn run build && find build -type f \\! -iname '*.gz' -exec gzip -k \\{\\} + && tar -cvf ../frontend.tar -C build/ ."
