...
const dateStrToSeconds = (dateStr: string) => new Date(dateStr).getTime() / 1000;
const numBuckets = 200;
const strokeOpacity = 0.03;

const Timeline: React.FC<TimelineProps> = ({ pictures }) => {
  ...
  const maxTime = pictures.length ? dateStrToSeconds(pictures[pictures.length - 1].taken_at!) : 0;
  ...
  return (
    ...
    >
      {pictures.map(picture => {
        ...
        return <line x1={x} y1={0} x2={x} y2={1} stroke="white" style={{ strokeOpacity }} />;
      })}
    </svg>
  );
};
