...
type Kind =
  "v4_photo" | "v4_album" | "legacy_series" | "legacy_picture" | "legacy_album";
...
/**
 * One round trip over every `path` index. Precedence: v4 photo, v4 album, legacy series, legacy
 * picture, legacy album (the spec's order, with series where Django checks them).
 */
export async function resolvePath(path: string): Promise<Resolution | null> {
  ...
  switch (row.kind) {
    ...
    case "legacy_series":
      return { kind: "series", seriesId: Number(row.id), path };
    case "legacy_picture":
      return {
        kind: "photo",
        source: "legacy",
